...
export function ScoreList({
  ...
  if (rows.length === 0) {
    return <p className="score-empty">No win scores yet</p>
  }
      ...
      <table className="score-table">
        ...
        <tbody>
          {rows.map((row, i) => {
            ...
            return (
              <tr key={row.id} className={isMe ? 'score-row score-row--me' : 'score-row'}>
                <td className="score-col-rank">{i + 1}</td>
                {showPlayer && (
                  <td className="score-col-player">{isMe ? 'You' : (row.username ?? '—')}</td>
                )}
