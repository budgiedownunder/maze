...
| `GET`    | `/api/v1/game-definitions` | Either | List game definitions, ordered by name. `scope=visible` (default) is the caller's visible set — their own (all visibilities) + shared-with-them + public + curated, de-duplicated; `scope=mine` is only their own (any visibility), optionally filtered by a case-insensitive name substring `q`. Paged via `limit` (default 20, capped at 100) / `offset`; the response echoes the effective `limit`/`offset` and a `hasMore` flag. |
