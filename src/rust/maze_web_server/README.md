...
| `PUT`    | `/api/v1/game-collections/{id}` | Either | Update a collection's metadata (`name`, `description`, `visibility`, `playMode`) the caller owns — or, for an **admin**, any collection (admin-override, ownership preserved). Membership + image unchanged. Setting `visibility = "curated"` requires an admin; a curated↔non-curated change appends to / removes from the featured catalogue. |
