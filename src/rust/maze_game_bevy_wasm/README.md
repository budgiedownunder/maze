...
`levels.lightsBelow` / `levels.lightsAbove` (`/game/?lights=<below>,<above>`) narrow the **point lights** alone, leaving every floor drawn — and never reach wider than the range above, since a floor nobody draws is never lit either. A shadowless point light measured about 7 ms a frame on an iPhone, and a maze spawns one per key and per treasure with no budget, so a tall stack with all its floors visible carries dozens of them. Narrowing the lights keeps the stack's shape and its content while dropping the cost that was measured; the objects still glow, because their meshes are emissive.
