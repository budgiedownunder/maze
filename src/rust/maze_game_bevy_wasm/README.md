...
`mobileMode` (`/game/?mobile_mode=1`) runs with the settings a phone needs, as one switch rather than a handful of parameters: the player's own floor drawn and animated, and no point light on the keys, treasures or finish orb (their meshes are emissive, so they still glow). The floor **above** is drawn as well while the maze is small enough for both to fit a measured cell budget. The window reaches upward rather than both ways because that is the direction of travel — the floor below holds nothing the player still needs, while the floor above is where the finish is, and drawing it is what keeps a ladder coherent. That floor is drawn but not lit: the lights keep their own, narrower range. On a maze too large for both — a 40x40 stack drawn two floors deep sees an iPhone's run cut short within a few turns, the device swamped or the platform stepping in — the floor above is given up and interim finishes resolve to portals, since a ladder would climb into nothing. The judgement is made on the base level's footprint, which a tapered stack's upper floors only ever undercut. The switches that measured null on a device — render scale, MSAA, freezing the pool animation — are deliberately left alone. The MAUI app appends the parameter on iOS and Android, where it knows its platform for a fact; in a browser the host page judges for itself when the parameter is absent, and an explicit `?mobile_mode=0` or `=1` always wins over that judgement.
...
`allowLadders` (`/game/?ladders=0` to turn it off) decides whether an interim level may finish with a ladder; it defaults to on, and `mobileMode` leaves it on whenever the window it sets draws the floor a ladder climbs into. Turned off, the finish type resolves to a portal — and because the rig drawn, the transition animation, the hatch cut into the floor above and the hole in a roofed finish tile all read that one value, they follow without anything else being set. A ladder only makes sense when the floor above it is drawn, so this pairs with a level window that hides it.
