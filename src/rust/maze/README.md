...
The crate exports `pub const MAX_TOTAL_FEATURES: usize = 16` as the canonical cap on `'K'` + `'D'` cells in any maze. It also exports `MAX_MAZE_CELLS` as the ceiling on `rows` × `cols` for a generated maze — the same number the React and MAUI generate dialogs check against the server-reported store cap. It is an in-memory ceiling: saving into a store configured with a lower cap still fails at save. Every layer that produces or persists a maze — the generator, the WASM bindings, the React Generate dialog, the React editor save flow, and the server save endpoint — enforces the same cap up front so the solver's error path never fires for a maze produced through the supported tools.
...
A request whose `2 * door_count + spare_doors + spare_keys` exceeds `MAX_TOTAL_FEATURES` is refused with `Error::Generate` — each real door contributes one key *and* one door to the produced grid, so the budget counts doors twice. The sum is computed with checked arithmetic, so counts large enough to wrap are refused rather than sliding back under the cap.

Two `GeneratorOptions` values bound the work one call can ask for, and both are refused with `Error::Generate` before any carving starts. `max_retries` may not exceed `pub const MAX_GENERATION_RETRIES: usize = 100` (also the default when unset), since every attempt carves a whole maze and then solves it to measure the spine. `min_spine_length` may not exceed `row_count * col_count`, which no path can reach — left unchecked, such a request would fail every attempt and report only once the retries were exhausted. `max_retries: Some(0)` remains the "make no attempt" sentinel.
...
Keys are auto-collected by walking over them — stepping onto a `'K'` cell adds the key to the bag, clears the cell, and queues a `GameEvent::KeyCollected { cell, id }` that flushes on the next `tick`. (`MazeGame::pickup()` remains as the internal collect mechanism; an external call normally finds nothing left since the cell was already cleared on walk-over.) Doors open over real time rather than blocking permanently: holding against a locked door while carrying a key starts it opening, and `MazeGame::tick(dt_ms)` advances and completes the open (emitting `GameEvent::DoorOpened`). Collected items are read via `MazeGame::bag()`, doors via `MazeGame::doors()`, and uncollected keys via `MazeGame::keys()`. `MazeGame::seed_carried_bag(items)` seeds the bag with items carried in from a previous level (used by the multi-level 3D game); it deliberately leaves the score untouched, so carried keys aren't re-counted.
