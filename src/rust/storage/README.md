...
| `game_collection_items` | The ordered members of a collection (added in `0012_game_collections.sql`). Composite PK `(collection_id, definition_id)`, `sort_order` INTEGER. FK `collection_id` `ON DELETE CASCADE`; **no FK on `definition_id`** — membership is presentation, so a reference the server can't resolve (e.g. a member the viewer may not see) is simply filtered out at display. Because there is no FK, nothing removes these rows automatically: **`delete_game_definition` prunes the game from every collection listing it** (re-compacting the survivors' `sort_order`) so a collection's item count never outruns the members it can show. |
