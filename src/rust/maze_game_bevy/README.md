...
| `multilevel_portcullis` | a pool-free 2-level stack with a portcullis door on the bottom level (a key sits just below it), for inspecting the door frame. The top level's floor fully covers the bottom, so climb the ladder (far corner) and stand over the door cell, then look down at the floor: the lintel sits just below the surface when it's contained in the cell, or pokes up as a low beam when it isn't. Collect the key and open the gate first to also confirm the raised grille isn't visible from above. |
